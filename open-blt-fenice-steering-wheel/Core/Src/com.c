...
#if (BOOT_COM_CAN_ENABLE > 0)
  /* transmit the packet. note that len is limited to 8 in the plausibility
   * check, so cast is okay.
   */
...
#endif
#if (BOOT_COM_RS232_ENABLE > 0)
  /* transmit the packet. note that len is limited to 255 in the plausibility
   * check, so cast is okay.
   */
...
blt_bool ComIsConnected(void) {
  ...
  /* Is there an active XCP connection? This indicates that the communication
   * interface is in the connection state.
   */
  if (XcpIsConnected()) {
    result = BLT_TRUE;
  }
...
void ComDeferredInit(void) {
  ...
  /* the default internal timed backdoor mechanism should start its timer after
   * the communication interfaces are initialized. since a deferred
   * initialization was now performed, the backdoor timer should be restarted.
   */
