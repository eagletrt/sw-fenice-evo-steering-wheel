...
void NvmReinit(void) {
#if (BOOT_NVM_HOOKS_ENABLE > 0)
  /* give the application a chance to re-initialize a driver for operating on
   * NVM that is not by default supported by this driver.
   */
  NvmReinitHook();
#endif
...
blt_bool NvmWrite(blt_addr addr, blt_int32u len, blt_int8u *data) {
...
#if (BOOT_NVM_HOOKS_ENABLE > 0)
  /* give the application a chance to operate on memory that is not by default
  ...
  if (result == BLT_NVM_OKAY) {
    /* data was within range of the additionally supported memory and
     * succesfully programmed, so we are all done.
     */
    return BLT_TRUE;
  } else if (result == BLT_NVM_ERROR) {
    /* data was within range of the additionally supported memory and attempted
     * to be programmed, but an error occurred, so we can't continue.
     */
    return BLT_FALSE;
  }
#endif

  /* still here so the internal driver should try and perform the program
   * operation */
...
blt_bool NvmErase(blt_addr addr, blt_int32u len) {
  ...
  /* process the return code */
  if (result == BLT_NVM_OKAY) {
    /* address was within range of the additionally supported memory and
     * succesfully erased, so we are all done.
     */
    return BLT_TRUE;
  } else if (result == BLT_NVM_ERROR) {
    /* address was within range of the additionally supported memory and
     * attempted to be erased, but an error occurred, so we can't continue.
     */
    return BLT_FALSE;
  }
#endif

  /* still here so the internal driver should try and perform the erase
   * operation */
