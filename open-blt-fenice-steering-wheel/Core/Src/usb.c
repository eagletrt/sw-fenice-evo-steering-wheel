 ...
 ****************************************************************************************/
/* For a USB bootloader, the backdoor needs to stay open long enough for the USB
 * device to enumerate on the host PC. Therefore the default backdoor open time
 * needs to be extended. Note that this won't be long enough for a first time
 * USB driver install on the host PC. In this case the bootloader should be
 * started with the backup backdoor that uses, for example, a digital input to
 * force the bootloader to stay active. This can be implemented in
 * CpuUserProgramStartHook(). Feel free to shorten/lengthen this time for
 * finetuning. Note that adding this configuration macro to blt_conf.h overrides
 * the value here.
 */
...
void UsbInit(void) {
  ...
  fifoPipeBulkOUT.handle =
      UsbFifoMgrCreate(fifoPipeBulkOUT.data, FIFO_PIPE_SIZE);
...
blt_bool UsbReceivePacket(blt_int8u *data, blt_int8u *len) {
  static blt_int8u
      xcpCtoReqPacket[BOOT_COM_USB_RX_MAX_DATA + 1]; /* one extra for length */
...
void UsbTransmitPipeBulkIN(void) {
  ...
  for (byte_counter = 0; byte_counter < nr_of_bytes_for_tx_endpoint;
       byte_counter++) {
    /* obtain data from the fifo */
...
void UsbReceivePipeBulkOUT(blt_int8u epnum) {
  ...
  for (byte_counter = 0; byte_counter < USB_Rx_Cnt; byte_counter++) {
    ...
    result =
        UsbFifoMgrWrite(fifoPipeBulkOUT.handle, usbRxBufferPtr[byte_counter]);
    /* verify that the fifo wasn't full */
    ASSERT_RT(result == BLT_TRUE);
  }
  /* Prepare Out endpoint to receive next packet */
  USBD_LL_PrepareReceive(&hUsbDeviceFS, BULK_OUT_EP, USBD_Bulk_GetRxBufferPtr(),
...
static void UsbFifoMgrInit(void) {
  ...
  /* initialize handle for the last one and use null-pointer for the next free
   * fifo  */
  pbc1->handle = i;
