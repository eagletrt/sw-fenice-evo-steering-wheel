 ...
 ****************************************************************************************/
/* Extend the default time that the backdoor is open if firmware updates via
 * TCP/IP are supported. in this case an activation of the bootloader results in
 * an initialization of the ethernet MAC. when connected to the network via a
 * router this can take several seconds. Feel free to shorten/lengthen this time
 * for finetuning, Note that adding this configuration macro to blt_conf.h
...
/** \brief Boolean flag initialized such that the normal initialization via
 * NetInit() should be deferred. A called to NetDeferredInit() is need to do the
 * actual initialization of this module.
 */
static blt_bool netInitializationDeferred = BLT_TRUE;
#else
/** \brief Boolean flag initialized such that the normal initialization via
 * NetInit() proceeds as usual.
 */
...
void NetInit(void) {
  ...
  if (netInitializationDeferred == BLT_FALSE) {
...
#if (BOOT_COM_NET_DHCP_ENABLE == 0)
    /* set the IP address */
    uip_ipaddr(ipaddr, BOOT_COM_NET_IPADDR0, BOOT_COM_NET_IPADDR1,
               BOOT_COM_NET_IPADDR2, BOOT_COM_NET_IPADDR3);
    uip_sethostaddr(ipaddr);
    /* set the network mask */
    uip_ipaddr(ipaddr, BOOT_COM_NET_NETMASK0, BOOT_COM_NET_NETMASK1,
               BOOT_COM_NET_NETMASK2, BOOT_COM_NET_NETMASK3);
    uip_setnetmask(ipaddr);
    /* set the gateway address */
    uip_ipaddr(ipaddr, BOOT_COM_NET_GATEWAY0, BOOT_COM_NET_GATEWAY1,
               BOOT_COM_NET_GATEWAY2, BOOT_COM_NET_GATEWAY3);
    ...
    /* extend the time that the backdoor is open in case the default timed
     * backdoor mechanism is used.
     */
...
blt_bool NetReceivePacket(blt_int8u *data, blt_int8u *len) {
  /* no need to check for newly received packets if this module is not
   * initialized */
  if (netInitializedFlag == BLT_TRUE) {
  ...
  /* packet reception and transmission is completely handled by the
   * NetServerTask so always return BLT_FALSE here.
   */
  return BLT_FALSE;
...
void NetApp(void) {
  ...
  if (uip_connected()) {
    /* init the dto counter and reset the pending dto data length and transmit
     * related flags.
     */
    s->dto_counter = 1;
    s->dto_len = 0;
  ...
  if (uip_poll()) {
    /* check if there is a packet waiting to be transmitted. this is done via
     * polling because then it is possible to asynchronously send data.
     * otherwise data is only really send after a newly received packet was
     * received.
     */
  ...
  if (uip_newdata()) {
    /* only process the data if its length is not longer than expected.
     * otherwise just ignore it. XCP is request/response. this means that a new
     * requests should only be processed when the response the the previous
     * request was sent. new requests before the last response was sent can
     * therefore also be ignored.
     */
    if (((uip_datalen() - 4) <= BOOT_COM_NET_RX_MAX_DATA) &&
        (s->dto_tx_pending == BLT_FALSE)) {
      /* the first 4 bytes contain a counter value in which we are not really
       * interested */
