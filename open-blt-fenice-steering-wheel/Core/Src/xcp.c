...
static blt_int8u XcpVerifyKey(blt_int8u resource, blt_int8u *key,
                              blt_int8u len);
#endif
...
extern blt_int8u XcpVerifyKeyHook(blt_int8u resource, blt_int8u *key,
                                  blt_int8u len);
#endif
 ...
 * communication module can be added. In order to use the XCP protocol in the
 * custom communication module, this hook function needs to be implemented. If
 * the XCP protocol is not needed, then simply remove the xcp.c source from the
 * project.
 */
...
void XcpPacketReceived(blt_int8u *data, blt_int8u len) {
  ...
  (void)len;
#else
  /* give the hook function a chance to process this packet. A return value of
   * BLT_TRUE indicates that the hook function processed the packet and that no
  ...
  /* only continue if connected */
    ...
    /* return to make sure response packet is not send because we are not
     * connected */
    return;
  }
...
static void XcpCmdConnect(blt_int8u *data) {
  ...
  /* reject the connection if the file module is not idle. this means that a
   * firmware update from the locally attached storage is in progress
   */
 ...
 * seed/key security feature enabled. The default XCP behavior is deviated in
 * this situation. The deviation is such that all zero values are returned if
 * the seed/key security feature is enabled and the programming resource is not
 * yet unlocked.
 */
#if (XCP_SEED_KEY_PROTECTION_EN == 1)
  /* check if PGM resource is unlocked */
  if ((xcpInfo.protection & XCP_RES_PGM) == XCP_RES_PGM) {
    /* resource is locked so memory read access is denied. zero values are
     * returned instead.
     */
...
static void XcpCmdGetSeed(blt_int8u *data) {
  ...
  if (data[1] == 0) {
     ...
     */
    if (((data[2] & XCP_RES_CALPAG) > 0) &&
        ((data[2] & ~XCP_RES_CALPAG) == 0)) {
      resourceOK = 1;
    }
    ...
    if ((xcpInfo.protection & data[2]) == 0) {
      /* set the seed length to 0 to indicate that the resource is already
       * unlocked */
      xcpInfo.ctoData[1] = 0;
      /* set packet length */
      xcpInfo.ctoLen = 2;
  ...
  /* process the mode flag. 0 is first part of the seed, 1 is remainder of the
   * seed */
  if (data[1] == 0) {
  ...
  CpuMemCopy((blt_addr)(&xcpInfo.ctoData[2]), (blt_addr)seedCurrentPtr,
             seedCurrentLen);
...
static void XcpCmdUnlock(blt_int8u *data) {
  ...
  /* is this the start of a key reception? the first unlock message contains the
   * total length of the key and subsequent messages the remainder length. if
   * the received length is >= than the previously received remainder, it must
   * be the reception start of a new key.
   */
...
static void XcpCmdProgramMax(blt_int8u *data) {
  ...
  /* program the data */
  if (NvmWrite((blt_addr)xcpInfo.mta, XCP_CTO_PACKET_LEN - 1, &data[1]) ==
      BLT_FALSE) {
