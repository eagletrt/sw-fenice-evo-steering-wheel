 ...
 *         BOOT_BACKDOOR_ENTRY_TIMEOUT_MS to blt_conf.h with your desired
 * backdoor open time in milliseconds.
 */
...
/** \brief In certain scenarios it is desired to be able to extend the default
 * backdoor entry time at runtime. This variable holds the extension time in
 * milliseconds. Note that this value must be initialized to zero here and not
 * in function BackDoorInit(), because BackDoorInit() is one of the last
 * functions called in BootInit(). This order should not be changed otherwise
 * there is a chance that the timed backdoor partially or completely times out
 * during BootInit(). Initializing the variable here, allows function
 * BackDoorSetExtension() to be called before BackDoorInit() was called.
 */
...
void BackDoorInit(void) {
  ...
  if (BackDoorEntryHook() == BLT_FALSE) {
    ...
    /* the backdoor is open so we should check if a update from locally attached
     * storage is requested and, if so, start it.
     */
...
void BackDoorCheck(void) {
  ...
  /* check if a connection with the host was already established. in this case
   * the backdoor stays open anyway, so no need to check if it needs to be
   * closed.
   */
...
#endif
#if (BOOT_FILE_SYS_ENABLE > 0)
  /* check if the file module is busy, indicating that a firmware update through
   * the locally attached storage is in progress. in this case the backdoor
   * stays open anyway, so no need to check if it needs to be closed.
   */
  ...
  if (backdoorOpen == BLT_TRUE) {
    ...
    if (TimerGet() >= (BOOT_BACKDOOR_ENTRY_TIMEOUT_MS + backdoorExtensionTime +
                       backdoorOpenTime)) {
      /* close the backdoor */
      backdoorOpen = BLT_FALSE;
#if (BOOT_FILE_SYS_ENABLE > 0)
      /* during the timed backdoor no remote update request was detected. now do
       * one last check to see if a firmware update from locally attached
       * storage is pending.
       */
      if (FileHandleFirmwareUpdateRequest() == BLT_FALSE)
#endif
      {
        /* no firmware update requests detected, so attempt to start the user
         * program. this function does not return if a valid user program is
         * present.
         */
        CpuStartUserProgram();
      }
