...
#define FLASH_END_ADDRESS                                                      \
  (flashLayout[FLASH_TOTAL_SECTORS - 1].sector_start +                         \
   flashLayout[FLASH_TOTAL_SECTORS - 1].sector_size - 1)
/** \brief Offset into the user program's vector table where the checksum is
 * located. For this target it is set to the end of the vector table. Note that
 * the value can be overriden in blt_conf.h, because the size of the vector
 * table could vary. When changing this value, don't forget to update the
...
#error                                                                         \
    "BOOT_FLASH_VECTOR_TABLE_CS_OFFSET is set too high. It must be located in the first writable block."
#endif
 ...
 *  \details  Programming is done per block of max FLASH_WRITE_BLOCK_SIZE. for
 * this a flash block manager is implemented in this driver. this flash block
 * manager depends on this flash block info structure. It holds the base address
 * of the flash block and the data that should be programmed into the flash
 *            block. The .base_addr must be a multiple of
...
static tFlashBlockInfo *FlashSwitchBlock(tFlashBlockInfo *block,
                                         blt_addr base_addr);
 ...
 ****************************************************************************************/
/** \brief   If desired, it is possible to set BOOT_FLASH_CUSTOM_LAYOUT_ENABLE
 * to > 0 in blt_conf.h and then implement your own version of the flashLayout[]
 * table in a source-file with the name flash_layout.c. This way you customize
 * the flash memory size reserved for the bootloader, without having to modify
 *           the flashLayout[] table in this file directly. This file will then
 * include flash_layout.c so there is no need to compile it additionally with
 * your project.
 */
...
#error "BOOT_NVM_SIZE_KB < 1024 is currently not supported."
    ...
    // { 0x08000000, 0x20000, 0, 1 },       /* flash sector idx 0 - reserved for
    // bootloader */
 ...
 ****************************************************************************************/
/** \brief   Local variable with information about the flash block that is
 * currently being operated on. \details The smallest amount of flash that can
 * be programmed is FLASH_WRITE_BLOCK_SIZE. A flash block manager is implemented
 * in this driver and stores info in this variable. Whenever new data should be
 * flashed, it is first added to a RAM buffer, which is part of this variable.
 * Whenever the RAM buffer, which has the size of a flash block, is full or data
 * needs to be written to a different block, the contents of the RAM buffer are
 *           programmed to flash. The flash block manager requires some software
 *           overhead, yet results is faster flash programming because data is
 * first harvested, ideally until there is enough to program an entire flash
 * block, before the flash device is actually operated on.
 */
static tFlashBlockInfo blockInfo;

/** \brief   Local variable with information about the flash boot block.
 *  \details The first block of the user program holds the vector table, which
 * on the STM32 is also the where the checksum is written to. Is it likely that
 *           the vector table is first flashed and then, at the end of the
 * programming sequence, the checksum. This means that this flash block need to
 * be written to twice. Normally this is not a problem with flash memory, as
 * long as you write the same values to those bytes that are not supposed to be
 * changed and the locations where you do write to are still in the erased 0xFF
 * state. Unfortunately, writing twice to flash this way, does not work reliably
 * on all micros. This is why we need to have an extra block, the bootblock,
 *           placed under the management of the block manager. This way is it
 * possible to implement functionality so that the bootblock is only written to
 * once at the end of the programming sequence.
 */
...
/************************************************************************************/ /**
                                                                                        ...
                                                                                        ****************************************************************************************/
void FlashInit(void) {
  /* init the flash block info structs by setting the address to an invalid
   * address */
...
blt_bool FlashErase(blt_addr addr, blt_int32u len) {
  ...
  /* only continue if all is okay so far */
  if (result == BLT_TRUE) {
    /* obtain the first and last sector entry indices to the flashLayout[]
     * array. */
...
blt_bool FlashWriteChecksum(void) {
  ...
  /* for the STM32 target we defined the checksum as the Two's complement value
   * of the sum of the first 7 exception addresses.
   *
   ...
   *
   *    the bootloader writes this 32-bit checksum value right after the vector
   * table of the user program. note that this means one extra dummy entry must
   * be added at the end of the user program's vector table to reserve storage
  ...
  if (bootBlockInfo.base_addr != FLASH_INVALID_ADDRESS) {
#if (BOOT_FLASH_CRYPTO_HOOKS_ENABLE > 0)
    /* perform decryption of the bootblock, before calculating the checksum and
    ...
    /* only continue if all is okay so far */
    if (result == BLT_TRUE) {
      /* compute the checksum. note that the user program's vectors are not yet
       * written to flash but are present in the bootblock data structure at
       * this point.
       */
      ...
      /* write the checksum */
      result = FlashWrite(flashLayout[0].sector_start +
...
blt_bool FlashVerifyChecksum(void) {
  ...
  /* add the checksum value that was written by FlashWriteChecksum(). Since this
   * was a Two complement's value, the resulting value should equal 0.
   */
  signature_checksum += *((blt_int32u *)(flashLayout[0].sector_start +
                                         BOOT_FLASH_VECTOR_TABLE_CS_OFFSET));
...
static tFlashBlockInfo *FlashSwitchBlock(tFlashBlockInfo *block,
  ...
  /* check if a switch needs to be made away from the boot block. in this case
   * the boot block shouldn't be written yet, because this is done at the end of
   * the programming session by FlashDone(), this is right after the checksum
   * was written.
   */
  ...
  /* check if a switch back into the bootblock is needed. in this case the
   * generic block doesn't need to be written here yet.
   */
  ...
  /* no switching between the generic block and the bootblock needed. it is a
   * switch within a generic block. the current block needs to be first
   * programmed before a switch to the new one can be make.
   */
  else {
    /* start by initializing the result to success */
    result = block;
    /* need to switch to a new block, so program the current one and init the
     * next */
...
static blt_bool FlashAddToBlock(tFlashBlockInfo *block, blt_addr address,
  ...
  current_base_addr =
      (address / FLASH_WRITE_BLOCK_SIZE) * FLASH_WRITE_BLOCK_SIZE;
  ...
  if (result == BLT_TRUE) {
    ...
    do {
      ...
      if ((blt_addr)(dst - &(block->data[0])) >= FLASH_WRITE_BLOCK_SIZE) {
        /* need to switch to a new block, so program the current one and init
         * the next */
        block =
            FlashSwitchBlock(block, current_base_addr + FLASH_WRITE_BLOCK_SIZE);
        if (block == BLT_NULL) {
...
static blt_bool FlashWriteBlock(tFlashBlockInfo *block) {
  ...
  /* note that the bootblock is already decrypted in FlashWriteChecksum(), if
   * the internal checksum mechanism is used. Therefore don't decrypt it again.
   */
  if (block != &bootBlockInfo)
#endif
  {
  ...
  if (result == BLT_TRUE) {
    ...
    for (word_cnt = 0; word_cnt < (FLASH_WRITE_BLOCK_SIZE / word_size);
         word_cnt++) {
      ...
      for (byte_idx = 0; byte_idx < word_size; byte_idx++) {
        /* compare the byte value located in flash with the one that was
         * supposed to be programmed there
         */
        if (((blt_int8u *)prog_addr)[byte_idx] != prog_data[byte_idx]) {
          /* byte value in flash is not as expected, so a programming error must
           * have happened
           */
          result = BLT_FALSE;
          break;
...
static blt_bool FlashEraseSectors(blt_int8u first_sector_idx,
  ...
  if (result == BLT_TRUE) {
    ...
    for (sectorIdx = first_sector_idx; sectorIdx <= last_sector_idx;
         sectorIdx++) {
      ...
      if (FlashEmptyCheckSector(sectorIdx) == BLT_FALSE) {
        ...
        if (HAL_FLASHEx_Erase(&eraseInitStruct,
                              (uint32_t *)&eraseSectorError) != HAL_OK) {
