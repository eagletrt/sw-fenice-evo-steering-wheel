...
/** \brief Structure type for grouping FATFS related objects used by this
 * module. */
typedef struct {
...
static blt_char *FileLibByteToHexString(blt_int8u byte_val,
                                        blt_char *destination);
static blt_char *FileLibLongToIntString(blt_int32u long_val,
                                        blt_char *destination);
#endif
...
/** \brief Local variable for storing information regarding the memory erase
 * operation.*/
...
blt_bool FileHandleFirmwareUpdateRequest(void) {
  ...
  /* make sure that there is no connection with a remote host to prevent two
   * firmware updates happening at the same time
   */
  if (ComIsConnected() == BLT_TRUE) {
    return BLT_FALSE;
  }
#endif
  /* a new firmware update request can only be handled if not already busy with
   * one */
  if (firmwareUpdateState != FIRMWARE_UPDATE_STATE_IDLE) {
    return BLT_FALSE;
  }
  ...
  if (FileIsFirmwareUpdateRequestedHook() == BLT_TRUE) {
    /* transition from IDLE to STARTING state, which kicks off the update
     * sequence */
...
void FileTask(void) {
  ...
  /* ------------------------------- starting
    ...
    if (f_open(&fatFsObjects.file, FileGetFirmwareFilenameHook(),
    ...
    FileFirmwareUpdateLogHook(
        "Parsing firmware file to detect erase blocks...");
#endif
  ...
  /* ------------------------------- erasing
    ...
    read_line_ptr = f_gets(lineParseObject.line, sizeof(lineParseObject.line),
                           &fatFsObjects.file);
    ...
    /* parse the S-Record line without copying the data values if the line is
     * not empty */
    if (read_line_ptr != BLT_NULL) {
      parse_result = FileSrecParseLine(lineParseObject.line,
                                       &lineParseObject.address, BLT_NULL);
    ...
    if (parse_result > 0) {
      ...
      if (eraseInfo.total_size == 0) {
        ...
        /* does this data fit at the end of the previously detected program
         * block? */
        if (lineParseObject.address ==
            (eraseInfo.start_address + eraseInfo.total_size)) {
 ...
 */
          ...
          FileLibByteToHexString((blt_int8u)(eraseInfo.start_address >> 24),
                                 &loggingStr[0]);
          FileLibByteToHexString((blt_int8u)(eraseInfo.start_address >> 16),
                                 &loggingStr[2]);
          FileLibByteToHexString((blt_int8u)(eraseInfo.start_address >> 8),
                                 &loggingStr[4]);
          FileLibByteToHexString((blt_int8u)eraseInfo.start_address,
                                 &loggingStr[6]);
          FileFirmwareUpdateLogHook(loggingStr);
          FileFirmwareUpdateLogHook("...");
#endif
          /* still here so we are ready to perform the memory erase operation */
          if (NvmErase(eraseInfo.start_address, eraseInfo.total_size) ==
              BLT_FALSE) {
            /* cannot continue with firmware update so go back to idle state */
            firmwareUpdateState = FIRMWARE_UPDATE_STATE_IDLE;
#if (BOOT_FILE_LOGGING_ENABLE > 0)
    ...
    if (f_eof(&fatFsObjects.file) > 0) {
      ...
      /* still here so we are ready to perform the last memory erase operation,
       * if there is still something left to erase.
       */
      if (eraseInfo.total_size > 0) {
#if (BOOT_FILE_LOGGING_ENABLE > 0)
        FileFirmwareUpdateLogHook("OK\n\r");
  ...
  /* ------------------------------- programming
    ...
    if (read_line_ptr != BLT_NULL) {
      parse_result = FileSrecParseLine(
          lineParseObject.line, &lineParseObject.address, lineParseObject.data);
      /* check parsing result */
      if (parse_result == ERROR_SREC_INVALID_CHECKSUM) {
        /* cannot continue with firmware update so go back to idle state */
        firmwareUpdateState = FIRMWARE_UPDATE_STATE_IDLE;
    ...
    if (parse_result > 0) {
      ...
      /* convert address to hex-string  */
      FileLibByteToHexString((blt_int8u)(lineParseObject.address >> 24),
                             &loggingStr[0]);
      FileLibByteToHexString((blt_int8u)(lineParseObject.address >> 16),
                             &loggingStr[2]);
      FileLibByteToHexString((blt_int8u)(lineParseObject.address >> 8),
                             &loggingStr[4]);
      FileLibByteToHexString((blt_int8u)lineParseObject.address,
                             &loggingStr[6]);
      FileFirmwareUpdateLogHook(loggingStr);
      FileFirmwareUpdateLogHook("...");
#endif
      ...
      if (NvmWrite(lineParseObject.address, parse_result,
                   lineParseObject.data) == BLT_FALSE) {
        /* cannot continue with firmware update so go back to idle state */
        firmwareUpdateState = FIRMWARE_UPDATE_STATE_IDLE;
#if (BOOT_FILE_LOGGING_ENABLE > 0)
        FileFirmwareUpdateLogHook("ERROR\n\r");
...
tSrecLineType FileSrecGetLineType(const blt_char *line) {
  ...
  if ((toupper((blt_int16s)(line[0])) != 'S') ||
      (isdigit((blt_int16s)(line[1])) == 0)) {
...
blt_bool FileSrecVerifyChecksum(const blt_char *line) {
  ...
  /* the checksum is calculated by summing up the values of the byte count,
   * address and databytes and then taking the 1-complement of the sum's least
   * signigicant byte */
  checksum = ~checksum;
...
/************************************************************************************/ /**
                                                                                        ...
                                                                                        **
                                                                                        ****************************************************************************************/
blt_int16s FileSrecParseLine(const blt_char *line, blt_addr *address,
                             blt_int8u *data) {
  ...
  case LINE_TYPE_S1:
    ...
    *address += FileLibHexStringToByte(line);
    ...
    data_byte_count =
        bytes_on_line - 3; /* -2 bytes address, -1 byte checksum */
  ...
  case LINE_TYPE_S2:
    ...
    *address += FileLibHexStringToByte(line);
    ...
    /* determine how many data bytes are on the line */
    data_byte_count =
        bytes_on_line - 4; /* -3 bytes address, -1 byte checksum */
    /* read and store data bytes if requested */
    if (data != BLT_NULL) {
      for (i = 0; i < data_byte_count; i++) {
        data[i] = FileLibHexStringToByte(line);
  ...
  case LINE_TYPE_S3:
    ...
    *address += FileLibHexStringToByte(line);
    ...
    /* determine how many data bytes are on the line */
    data_byte_count =
        bytes_on_line - 5; /* -4 bytes address, -1 byte checksum */
    /* read and store data bytes if requested */
    if (data != BLT_NULL) {
      for (i = 0; i < data_byte_count; i++) {
        data[i] = FileLibHexStringToByte(line);
