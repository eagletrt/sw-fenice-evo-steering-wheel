 ...
 ****************************************************************************************/
/** \brief CAN bittiming table for dynamically calculating the bittiming
 * settings. \details According to the CAN protocol 1 bit-time can be made up of
 * between 8..25 time quanta (TQ). The total TQ in a bit is SYNC + TSEG1 + TSEG2
 * with SYNC always being 1. The sample point is (SYNC + TSEG1) / (SYNC + TSEG1
 * + SEG2) * 100%. This array contains possible and valid time quanta
 * configurations with a sample point between 68..78%.
 */
...
static blt_bool CanGetSpeedConfig(blt_int16u baud, blt_int16u *prescaler,
  ...
  /* the CAN peripheral can be clocked by the external crystal, the PLL1Q or the
   * PLL2Q clock. the external crystal option is typically preferred to meet the
   * clock tolerance specified in the CAN 2.0B protocol. therefore, this driver
   * assumes that an external crystal is present. configure the external crystal
   * to be the clock source of the CAN peripheral.
   */
  ...
  for (cnt = 0; cnt < sizeof(canTiming) / sizeof(canTiming[0]); cnt++) {
    if ((canClockFreqkHz %
         (baud * (canTiming[cnt].tseg1 + canTiming[cnt].tseg2 + 1))) == 0) {
      /* compute the prescaler that goes with this TQ configuration */
      *prescaler = canClockFreqkHz /
                   (baud * (canTiming[cnt].tseg1 + canTiming[cnt].tseg2 + 1));
...
void CanInit(void) {
  ...
  /* the current implementation supports CAN1 and 2. throw an assertion error in
   * case a different CAN channel is configured.
   */
  ASSERT_CT(
      (BOOT_COM_CAN_CHANNEL_INDEX == 0 || BOOT_COM_CAN_CHANNEL_INDEX == 1));

  /* obtain bittiming configuration information. */
  if (CanGetSpeedConfig(BOOT_COM_CAN_BAUDRATE / 1000, &prescaler, &tseg1,
                        &tseg2) == BLT_FALSE) {
    /* Incorrect configuration. The specified baudrate is not supported for the
     * given clock configuration. Verify the following settings in blt_conf.h:
  ...
  /* FD mode is not used by this driver, so the .Init.DataXxx values are don't
   * care. */
  ...
  /* initialize the CAN controller. this only fails if the CAN controller
   * hardware is faulty. no need to evaluate the return value as there is
   * nothing we can do about a faulty CAN controller.
   */
  (void)HAL_FDCAN_Init(&canHandle);

  /* configure the reception filter. note that the implementation of this
   * function always returns HAL_OK as long as the CAN controller is
   * initialized, so no need to evaluate the return value.
   */
  if ((rxMsgId & 0x80000000) == 0) {
...
void CanTransmitPacket(blt_int8u *data, blt_int8u len) {
  ...
  if (status == HAL_OK) {
    ...
    /* only continue with polling for transmit completion if the message
     * transmit request could be submitted.
     */
    if (status == HAL_OK) {
      ...
      while (HAL_FDCAN_IsTxBufferMessagePending(&canHandle, FDCAN_TX_BUFFER0) !=
             0) {
        /* service the watchdog. */
        CopService();
        /* break loop upon timeout. this would indicate a hardware failure or no
...
blt_bool CanReceivePacket(blt_int8u *data, blt_int8u *len) {
  ...
  if (HAL_FDCAN_GetRxFifoFillLevel(&canHandle, FDCAN_RX_FIFO0) > 0) {
    ...
    rxStatus =
        HAL_FDCAN_GetRxMessage(&canHandle, FDCAN_RX_FIFO0, &rxMsgHeader, data);
  }
