 ...
 * Type definitions
 ****************************************************************************************/
/** \brief Define the uip_tcp_appstate_t datatype. This is the state of our
 * tcp/ip application, and the memory required for this state is allocated
 * together with each TCP connection. One application state for each TCP
 * connection.
 */
typedef struct net_state {
  blt_int32u dto_counter;
  blt_int8u
      dto_data[BOOT_COM_NET_TX_MAX_DATA + 4]; /* +4 for counter overhead */
  blt_int16u dto_len;
  blt_bool dto_tx_req;
