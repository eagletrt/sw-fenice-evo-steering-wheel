 ...
 * Configuration
 ****************************************************************************************/
/** \brief Maximum length of the transport layer's command transmit object
 * packet. */
...
/** \brief Name in string format that is used to identify the ECU to the XCP
 * master using the GET_ID command.
 */
...
/** \brief Enable (=1) or disable (=0) support for the calibration resource.
 * This is required when data is written to RAM during the XCP session.
 */
#define XCP_RES_CALIBRATION_EN (0)

/** \brief Enable (=1) or disable (=0) support for the paging resource. This is
 *         required when switching between application specific calibration
 * pages should be supported. In this case the application specific external
 *         functions AppCalSetPage and AppCalGetPage must be provided.
 */
#define XCP_RES_PAGING_EN (0)

/** \brief Enable (=1) or disable (=0) support for the programming resource.
 * This is required when non-volatile memory will be erased or programmed during
 * an XCP session.  In this case the following external hardware specific
 *         functions must be provided: NvmWrite, NvmErase and
 * CpuStartUserProgram.
 */
#define XCP_RES_PROGRAMMING_EN (1)

/** \brief Enable (=1) or disable (=0) support for the data acquisition
 * resource. This note that this feature is currently not supported by the XCP
 * driver.
 */
#define XCP_RES_DATA_ACQUISITION_EN (0)

/** \brief Enable (=1) or disable (=0) support for the data stimulation
 * resource. This note that this feature is currently not supported by the XCP
 * driver.
 */
#define XCP_RES_DATA_STIMULATION_EN (0)

/** \brief Enable (=1) or disable (=0) support for the seed/key protection
 * feature. If enabled, the XCP master has to perform a GET_SEED/UNLOCK sequence
 * to obtain access to a resource. The protection algorithm is implemented in
...
/** \brief Enable (=1) or disable (=0) uploading. By default, XCP always allows
 * memory read operations using the commands UPLOAD and SHORT_UPLOAD. This is
 * not always desired for security reasons. If disabled, memory reads via XCP
...
/** \brief Enable (=1) or disable the hook function that gets called each time
 * an XCP packet was received from the host. \details A master-slave bootloader
 * can be realized by using this hook-function. The mode parameter in the XCP
 * Connect command can be interpreted as a node ID. When trying to connect to a
 * slave, a gateway could be activated that passes the packet on to the slave.
 * When the response packet is received from the slave, ComTransmitPacket() can
 * be called to pass the response on to the host. At the end of a firmware
 * update procedure, the XCP Program Reset command is called, which can be used
 * to deactivate the gateway. If this hook-function returns BLT_TRUE, the packet
 * is no longer processed by the XCP module. If it returns BLT_FALSE, then the
 * packet is processed as usual by the XCP module.
 */
