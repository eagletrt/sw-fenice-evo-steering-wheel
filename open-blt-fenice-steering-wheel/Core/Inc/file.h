...
/** \brief Error code because in incorrect checksum was found in the firmware
 * file. */
#define FILE_ERROR_INVALID_CHECKSUM_IN_FILE (3)
/** \brief Error code because the file pointers read pointer could not be
 * rewinded. */
#define FILE_ERROR_REWINDING_FILE_READ_POINTER (4)
/** \brief Error code because an error occurred during the memory erase
 * operation. */
#define FILE_ERROR_CANNOT_ERASE_MEMORY (5)
/** \brief Error code because an error occurred during the memory write
 * operation. */
#define FILE_ERROR_CANNOT_PROGRAM_MEMORY (6)
/** \brief Error code because the program's checksum could not be written to
 * memory. */
#define FILE_ERROR_CANNOT_WRITE_CHECKSUM (7)

/** \brief Maximum number of characters that can be on a line in the firmware
 * file. */
#define MAX_CHARS_PER_LINE (256)
/** \brief Maximum number of data bytes that can be on a line in the firmware
 * file (S-record).
 */
#define MAX_DATA_BYTES_PER_LINE (MAX_CHARS_PER_LINE / 2)
/** \brief Return code in case an invalid checksum was detected on an S-record
 * line. */
...
typedef struct {
  ...
  blt_int8u
      data[MAX_DATA_BYTES_PER_LINE]; /**< array for S1, S2 or S3 data bytes*/
