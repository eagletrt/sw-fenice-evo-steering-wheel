...
inline void remove_trailing (char *buf) {
    ...
    bool is_minus = init == '-';
    if (!is_cifra && !is_capital_letter && !is_letter && !is_minus) buf[0] = ' ';
}
/*
...
void speed_update(primary_speed_converted_t *data) {
    ...
    if (data->inverter_l != speed_last_state.inverter_l || data->inverter_r != speed_last_state.inverter_r) {
...
void hv_current_update(primary_hv_current_converted_t *data) {
    if (data->current != hv_current_last_state.current) {
        ...
        lv_bar_set_value(steering.racing_hv_bar, hv_current_last_state.current, LV_ANIM_OFF);
    }
}
...
void lv_currents_update(primary_lv_currents_converted_t *data) {
    ...
    if (old_current_lv_battery != data->current_lv_battery) {
        ...
        lv_bar_set_value(steering.racing_lv_bar, data->current_lv_battery, LV_ANIM_OFF);
    }
}
...
void steering_angle_update(secondary_steering_angle_converted_t *data) {
    if (data->angle != steering_angle_converted.angle) {
        ...
        if (steering.curr_focus == STEER) {
            ...
            lv_slider_set_range(steering.slider, STEERING_ANGLE_RANGE_LOW, STEERING_ANGLE_RANGE_HIGH); // se range 45 e max value 180 -> set_value ( 0.25 * gradi_inclinazione )
            lv_slider_set_value(steering.slider, steering_angle_converted.angle, LV_ANIM_OFF);
        }
    }
}
...
void pedals_output_update(secondary_pedals_output_converted_t *data) {
    if (data->apps != pedals_output_last_state.apps) {
        ...
        if (steering.curr_focus == APPS) {
            ...
            lv_slider_set_value(steering.slider, pedals_output_last_state.apps, LV_ANIM_OFF);
        }
    }
