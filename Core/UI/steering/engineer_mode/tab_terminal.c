...
void init_terminal_tab_styles(void) {
  ...
  lv_style_set_text_color(&tab_terminal_style,
...
void tab_terminal_create(lv_obj_t *parent) {
  ...
  static lv_coord_t tab_terminal_cols_ratios[] = {SCREEN_WIDTH,
  ...
  for (int i = 0; i < TERMINAL_ROWS_N; i++) {
    ...
    lv_obj_set_grid_cell(tab_terminal_labels[i], LV_GRID_ALIGN_CENTER, 0, 1,
...
void tab_terminal_new_message(const char *message) {
  ...
  lv_label_set_text_fmt(tab_terminal_labels[tab_terminal_message_index], "%s",
                        message);
