...
void update_shutdown_circuit_component(Box *screen, shutdown_circuit_indexes_t idx, bool is_close) {
    Box *shut_box = get_box(screen, swoc_elems_n, swoc_sd);
    shut_box->updated = 1;
    ...
    if ((last_opend_index = last_shutdown_element_unknown()) != shutdown_circuit_no_element_index) {
        // devices are missing
        shut_box->default_fg_color = GRAPHICS_COLOR_WHITE;
        strcpy(shut_box->label->text, "not avail");
        return;
    }
    if ((last_opend_index = last_open_shutdown_circuit()) != shutdown_circuit_no_element_index) {
        ...
        strcpy(shut_box->label->text, shutdown_labels[last_opend_index]);
        if (strlen(shutdown_labels[last_opend_index]) > 10) {
            shut_box->label->font_size = 10;
        } else {
            shut_box->label->font_size = 20;
        }
        shut_box->default_fg_color = GRAPHICS_COLOR_BLACK;
        shut_box->default_bg_color = GRAPHICS_COLOR_RED;
    ...
    strcpy(shut_box->label->text, "OK");
    shut_box->label->font_size = 20;
    shut_box->default_fg_color = GRAPHICS_COLOR_BLACK;
    shut_box->default_bg_color = GRAPHICS_COLOR_GREEN;
    global_shutdown_status = SC_CLOSE;
}

void ecu_status_update(Box *screen, bool valid) {
    Box *ecu_box = get_box(screen, swoc_elems_n, swoc_status);
    ...
    switch (primary_ecu_status_last_state->status) {
        ...
        case primary_ecu_status_status_check_inv_settings: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "INIT");
            break;
        }
        case primary_ecu_status_status_idle: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "IDLE");
            break;
        }
        case primary_ecu_status_status_start_ts_precharge:
        case primary_ecu_status_status_wait_ts_precharge: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "PRCH");
            break;
        }
        case primary_ecu_status_status_wait_driver: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "SETUP");
            break;
        }
        case primary_ecu_status_status_enable_inv_drive: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "ENINV");
            break;
        }
        case primary_ecu_status_status_re_enable_inverter_drive: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "REINV");
            break;
        }
        case primary_ecu_status_status_drive: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "DRIVE");
            break;
        }
        ...
        case primary_ecu_status_status_wait_ts_discharge: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "TSOFF");
            break;
        }
        case primary_ecu_status_status_fatal_error: {
            snprintf(ecu_box->label->text, SWOC_STRING_LEN, "FATAL");
            break;
        }
    }
}
...
void ecu_power_maps_update(Box *screen, bool valid) {
    Box *torque_box = get_box(screen, swoc_elems_n, swoc_torque);
    Box *slip_box = get_box(screen, swoc_elems_n, swoc_slip);
    Box *regen_box = get_box(screen, swoc_elems_n, swoc_regen);
    ...
    torque_box->updated = 1;
    slip_box->updated = 1;
    regen_box->updated = 1;

    if (primary_ecu_power_maps_last_state->reg_state /* && primary_ecu_control_status_last_state->control_enabled */) {
        regen_box->default_bg_color = GRAPHICS_COLOR_GREEN;
        regen_box->default_fg_color = GRAPHICS_COLOR_BLACK;
    } else {
        regen_box->default_bg_color = GRAPHICS_COLOR_BLACK;
        regen_box->default_fg_color = GRAPHICS_COLOR_WHITE;
    }
    if (primary_ecu_power_maps_last_state->sc_state /* && primary_ecu_control_status_last_state->control_enabled */) {
        slip_box->default_bg_color = GRAPHICS_COLOR_GREEN;
        slip_box->default_fg_color = GRAPHICS_COLOR_BLACK;
    } else {
        slip_box->default_bg_color = GRAPHICS_COLOR_BLACK;
        slip_box->default_fg_color = GRAPHICS_COLOR_WHITE;
    }
    if (primary_ecu_power_maps_last_state->tv_state /* && primary_ecu_control_status_last_state->control_enabled */) {
        torque_box->default_bg_color = GRAPHICS_COLOR_GREEN;
        torque_box->default_fg_color = GRAPHICS_COLOR_BLACK;
    } else {
        torque_box->default_bg_color = GRAPHICS_COLOR_BLACK;
        torque_box->default_fg_color = GRAPHICS_COLOR_WHITE;
    }
}
...
void hv_soc_estimation_update(Box *screen, bool valid) {
    Box *soc_box = get_box(screen, swoc_elems_n, swoc_soc_hv);
    ...
    soc_box->value->value = soc * 100.0f;
    soc_box->updated = 1;
}

void lv_soc_estimation_update(Box *screen, bool valid) {
    Box *soc_box = get_box(screen, swoc_elems_n, swoc_soc_lv);
    ...
    soc_box->value->value = soc * 100.0;
    soc_box->updated = 1;
}

void hv_cells_temp_stats_update(Box *screen, bool valid) {
    Box *cells_temp = get_box(screen, swoc_elems_n, swoc_temp_hv);
    ...
    cells_temp->value->value = tmax;
    cells_temp->updated = 1;
}
...
void tlm_lap_time_update(Box *screen, bool valid) {
    // component is swoc_lap_time
    Box *lap_time = get_box(screen, swoc_elems_n, swoc_lap_time);

    GET_LAST_STATE(secondary, tlm_laps_stats, SECONDARY, TLM_LAPS_STATS);
    lap_time->updated = 1;
    lap_time->value->value = secondary_tlm_laps_stats_last_state->last_time;
    if (secondary_tlm_laps_stats_last_state->last_time < secondary_tlm_laps_stats_last_state->best_time) {
        lap_time->default_fg_color = GRAPHICS_COLOR_BLACK;
        lap_time->default_bg_color = GRAPHICS_COLOR_PURPLE;
        lap_time->default_fg_color = GRAPHICS_COLOR_BLACK;
        lap_time->default_bg_color = GRAPHICS_COLOR_PURPLE;
    } else if (secondary_tlm_laps_stats_last_state->last_time <= last_lap) {
        lap_time->default_fg_color = GRAPHICS_COLOR_BLACK;
        lap_time->default_bg_color = GRAPHICS_COLOR_GREEN;
        lap_time->default_fg_color = GRAPHICS_COLOR_BLACK;
        lap_time->default_bg_color = GRAPHICS_COLOR_GREEN;
    } else {
        lap_time->default_fg_color = GRAPHICS_COLOR_BLACK;
        lap_time->default_bg_color = GRAPHICS_COLOR_YELLOW;
        lap_time->default_fg_color = GRAPHICS_COLOR_BLACK;
        lap_time->default_bg_color = GRAPHICS_COLOR_YELLOW;
    }
...
void inv_l_rcv_update(Box *screen, bool valid) {
    Box *mot_box = get_box(screen, swoc_elems_n, swoc_temp_mot);
    Box *inv_box = get_box(screen, swoc_elems_n, swoc_temp_inv);
    ...
    if (r_motor_temp != INVERTER_MESSAGE_UNINITIALIZED) {
        mot_box->updated = 1;
        mot_box->value->value = motor_temp_mean;
    }
    ...
    if (r_igbt_temp != INVERTER_MESSAGE_UNINITIALIZED) {
        inv_box->updated = 1;
        inv_box->value->value = igbt_temp_mean;
    }
}
...
void lv_pumps_speed_update(Box *screen, bool valid) {
    Box *cool_box = get_box(screen, swoc_elems_n, swoc_pt_cooling);
    ...
    if (primary_lv_pumps_speed_last_state->status == primary_lv_pumps_speed_status_auto) {
        snprintf(cool_box->label->text, SWOC_STRING_LEN, "A");
        cool_box->default_bg_color = GRAPHICS_COLOR_GREEN;
        cool_box->default_fg_color = GRAPHICS_COLOR_BLACK;
    } else {
        snprintf(cool_box->label->text, SWOC_STRING_LEN, "%.0f", primary_lv_pumps_speed_last_state->pumps_speed * 10.0f);
        cool_box->default_bg_color = GRAPHICS_COLOR_BLACK;
        cool_box->default_fg_color = GRAPHICS_COLOR_WHITE;
    }
}
...
void ptt_periodic_check(Box *screen) {
    Box *ptt_box = get_box(screen, swoc_elems_n, swoc_ptt);
    ...
    if ((primary_ecu_ptt_status_last_state->status == primary_ecu_ptt_status_status_off) && ptt_button_pressed) {
        ...
        snprintf(ptt_box->label->text, SWOC_STRING_LEN, "MUTE");
        ptt_box->default_bg_color = GRAPHICS_COLOR_BLACK;
        ptt_box->default_fg_color = GRAPHICS_COLOR_WHITE;
        ...
        primary_ecu_set_ptt_status_last_state->status = primary_ecu_set_ptt_status_status_on;
        snprintf(ptt_box->label->text, SWOC_STRING_LEN, "TALK");
        ptt_box->default_bg_color = GRAPHICS_COLOR_BLUE;
        ptt_box->default_fg_color = GRAPHICS_COLOR_WHITE;
