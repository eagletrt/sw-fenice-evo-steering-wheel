...
uint8_t* flashAddress = (uint8_t*)0x08060000;
...
void CrashCatcher_Entry(const CrashCatcherExceptionRegisters* pExceptionRegisters)
{
    ...
    do
    {
        ...
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--Signature\r\n", 13, 10);
        dumpSignature();
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--Flags\r\n", 9, 10);
        dumpFlags(&object);
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--R0-R3\r\n", 9, 10);
        dumpR0toR3(&object);
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--R4-R11\r\n", 10, 10);
        dumpR4toR11(&object);
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--R12\r\n", 7, 10);
        dumpR12(&object);
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--SP\r\n", 6, 10);
        dumpSP(&object);
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--LR-PC-PSR\r\n", 13, 10);
        dumpLR_PC_PSR(&object);
        // HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--MSP-PSP-ExceptionPSR\r\n", 24, 10);
        dumpMSPandPSPandExceptionPSR(&object);
        if (object.flags & CRASH_CATCHER_FLAGS_FLOATING_POINT) {
            HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"--FloatingPoinRegisters\r\n", 25, 10);
            dumpFloatingPointRegisters(&object);
        }
        // HAL_UART_Transmit(&huart2, (const uint8_t*)"--MemoryRegions\r\n", 17, 10);
        dumpMemoryRegions(CrashCatcher_GetMemoryRegions());
        if (!isARMv6MDevice()) {
            // HAL_UART_Transmit(&huart2, (const uint8_t*)"--FaultStatusRegisters\r\n", 24, 10);
...
static void dumpMemoryRegions(const CrashCatcherMemoryRegion* pRegion)
{
    while (pRegion && pRegion->startAddress != 0xFFFFFFFF)
    {
        ...
        // HAL_UART_Transmit(&huart2, (const uint8_t*)"--MemoryRegion\r\n", 16, 10);
        CrashCatcher_DumpMemory(pRegion, CRASH_CATCHER_BYTE, 2 * sizeof(uint32_t));
        // HAL_UART_Transmit(&huart2, (const uint8_t*)"--MemoryContent\r\n", 17, 10);
...
static void dumpHexDigit(uint8_t nibble)
{
    ...
    HAL_UART_Transmit(&hlpuart1, (const uint8_t *)(hexToASCII + nibble), 1, 10);
}
...
void CrashCatcher_DumpMemory(const void* pvMemory, CrashCatcherElementSizes elementSize, size_t elementCount) {
    ...
    HAL_UART_Transmit(&hlpuart1, (const uint8_t*)"\r\n", 2, 10);
}
...
const CrashCatcherMemoryRegion* CrashCatcher_GetMemoryRegions(void) {
  static const CrashCatcherMemoryRegion regions[] = {
                {0x20000000, 0x20000000 + 128 * 1024, CRASH_CATCHER_BYTE},
                {0x24000000, 0x24000000 + 320 * 1024, CRASH_CATCHER_BYTE},
                {0x30000000, 0x30000000 + 32 * 1024, CRASH_CATCHER_BYTE},
                {0x38000000, 0x38000000 + 16 * 1024, CRASH_CATCHER_BYTE},
                {0x00000000, 0x00000000 + 64 * 1024, CRASH_CATCHER_BYTE},
                {0xFFFFFFFF, 0xFFFFFFFF, CRASH_CATCHER_BYTE}
                };
            return regions;
